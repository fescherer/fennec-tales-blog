...
export function PostCard({ post }: PostCardProps) {
  ...
  return (
    // <Link href={`/posts/${post.category}/${post.slug}`} className="group relative flex w-full flex-col rounded-xs border border-transparent bg-secondary transition-all hover:border-secondary-border" title={post.title}>
    ...
    //       <h2 className="mb-2 line-clamp-3 text-lg font-medium text- transition-colors group-hover:text-accent">{post.title}</h2>
    ...
    <Link href={`/posts/${post.category}/${post.slug}`} className="card bg-base-100 shadow-sm transition-all hover:scale-105">
      {isNew && <Ribbon title="New" />}
      <figure>
        <Image src={post.image} width={720} height={404} alt="image" />
      </figure>
